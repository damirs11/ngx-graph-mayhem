...
export class ColaForceDirectedLayout implements Layout {
  ...
  internalGraphToOutputGraph(internalGraph: any): Graph {
    this.outputGraph.nodes = internalGraph.nodes.map((node: { id: any; x: number; y: number; dimension: { width: any; height: any; }; }) => ({
      ...node,
      id: node.id || id(),
      position: {
        x: node.x,
    ...
    this.outputGraph.edges = internalGraph.links
      .map((edge: { source: number | InputNode; target: number | InputNode; }) => {
      ...
      .concat(
        internalGraph.groupLinks.map((groupLink: { source: any; target: any; }) => {
          const sourceNode = internalGraph.nodes.find((foundNode: any) => (foundNode as any).id === groupLink.source);
          const targetNode = internalGraph.nodes.find((foundNode: any) => (foundNode as any).id === groupLink.target);
          const source =
            sourceNode || internalGraph.groups.find((foundGroup: any) => (foundGroup as any).id === groupLink.source);
          const target =
            targetNode || internalGraph.groups.find((foundGroup: any) => (foundGroup as any).id === groupLink.target);
          return {
            ...groupLink,
            source: source.id,
            target: target.id,
    ...
    this.outputGraph.clusters = internalGraph.groups.map((group: { bounds: { width: () => number; height: () => number; x: number; y: number; }; }, index: string | number): ClusterNode => {
      // @ts-ignore
